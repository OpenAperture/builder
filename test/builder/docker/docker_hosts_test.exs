defmodule OpenAperture.Builder.DockerHostsTests do
  ...
  test "next_available - 1 invalid host" do
  	:meck.new(EtcdCluster, [:passthrough])
  	:meck.expect(EtcdCluster, :get_hosts, fn _ -> [%FleetApi.Machine{}] end)
  ...
  test "next_available - 1 host" do
  	:meck.new(EtcdCluster, [:passthrough])
  	:meck.expect(EtcdCluster, :get_hosts, fn _ -> [%FleetApi.Machine{primaryIP: "123.456.789.0123"}] end)
  ...
  test "next_available - 2 host" do
  	:meck.new(EtcdCluster, [:passthrough])
  	:meck.expect(EtcdCluster, :get_hosts, fn _ -> [%FleetApi.Machine{primaryIP: "123.456.789.0123"}, %FleetApi.Machine{primaryIP: "234.456.789.0123"}] end)
